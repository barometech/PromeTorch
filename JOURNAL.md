...
## 2026-07-21: sync + гигиена репо + RESULTS re-замер (Gap #2 закрыт) + phi3-диагноз

Сессия «доведи репозиторий до хорошего состояния». GPU был свободен.

**Git-sync (было главное расхождение):** локальный `main` опережал GitHub на
7 коммитов (22 июня + 3 июля: gemma3 GeGLU GPU, promeserve race, license BSD,
аудит-доки). Origin застыл на `f8034ca` (11 июня). Запушено → синхронизировано.
Эльбрус (`ELBRUS_HOST`, up 62 дня, 32 ядра) отставал на 23 коммита — `git pull` до
`13ec8a5`, его собственные коммиты (SINQ A/B, Q5_K k-slice) все уже были в origin,
потерь нет.

**Гигиена репо:**
- Закоммичены 4 забытых integration-теста `python/tests/test_*.py` (по .gitignore
  обязаны быть tracked) + 2 research-отчёта `docs/research/*.md` (~2500 строк).
- Снесены 19 orphan agent-worktrees `.claude/worktrees/agent-*` + их ветки
  (8c3bea8 в main; невлитый 991bfd4 = journal-запись «936 tok/s», её текст уже
  12× в JOURNAL — потерь нет).

**RESULTS.md re-замер (Gap #2 → ЗАКРЫТ):** формальный прогон A100, 200 ток,
5-run median, greedy + Ollama baseline того же дня:

| Модель | PromeTorch | Ollama | Ratio | Было (апр) |
|--------|-----------:|-------:|------:|-----------:|
| qwen3:4b | **100.6** | 189.5 | 53% | 82.6 (+22%) |
| gemma3:4b | **87.7** | 116.9 | 75% | 81.4 (+8%) |
| deepseek-r1:8b | **57.4** | 120.0 | 48% | 51.1 (+12%) |

Прирост — от per-token фиксов (F16 attn_v `a42d2ac`, NeoX RoPE `d25b95b`,
gemma3 GeGLU `85eccc0`/`e25a4ba`), не от железа. Все выходы связные, без
...
**Phase 3 — warp-shuffle softmax в flash_decode (`cf7ff95`):**
- flash_decode (~10% decode) считал softmax max/sum серийным tid==0 циклом по
  chunk_len≤256, пока 127+ потоков простаивали. Заменил на warp-shuffle блочную
  редукцию (`block_reduce_max/sum`) в 3 ядрах: partial обычное + partial graph
  (горячий decode-путь!) + reduce.
- **Урок про __syncthreads:** первая версия дала −1.3% (109→107.7) — параллельная
  редукция добавила 4 лишних sync (7 против 3). Убрал хвостовые sync в хелперах
  (вызывающий и так синхронизируется) → 5 sync → +0.5%. Синхронизация дороже, чем
  кажется; параллелизм без контроля sync проигрывает серийному на коротких данных.
...
* **[коллега] 8C v4** — впервые увидели реальное v4 железо. Раньше нет
...
2. [коллега] 8C v4 — валидация `pmaddubsh` SIMD path (~4-6 tok/s эстимат)
...
**Сейчас в работе:** rebuild на Эльбрусе lemur-1 (e2k-8c2 v5, LCC 1.29.16, 32 ядра) для проверки T1-T8 fix'ов вживую + последующий запуск #78/#109/#129. NM Quad host (NMQUAD_HOST:4079) недоступен — #114/#123/#128 ждут хоста.
...
| Модель | SP tok/s | TP-4 tok/s | Статус |
|---|---|---|---|
| phi3.5-mini | 3.5 | **6.4** | ✅ RU + EN |
| qwen3-1.7B | (high) | 17.1 | ✅ RU |
| **qwen3-4B** | 5.5 | **10.9** | ✅ RU + CoT (почти baseline 11.4) |
| **mistral-7B** | 2.9 | **8.5** | ✅ RU |
| qwen2.5-7B | (works SP) | OOM | ✅ RU |
| qwen3-8B | (works) | TBD | ✅ |
| **gemma3-4B** | 4.7 | n/a (post_norm) | ✅ markdown RU |

**Major win:** qwen3-4B TP-4 теперь 10.9 tok/s с правильным русским
(было 8.0). Baseline 11.4 был с broken Russian. Speed + quality
вместе — практически достигнуто.

Output samples (verified live 2026-05-03 18:32-18:46 МСК):
* phi3 RU SP: «Космос - это всеобъемлющая область пространства, включающая
  все газопылевые объекты и их окружения. Он охватывает все извест...»
* phi3 EN SP: «Moscow, the capital city of Russia, is not only the
  political heart of the country but also a vibrant cultural and
  economic center. With its rich history that dates back to 1...»
* phi3 RU TP-4: «Космос - это всеобъемлющий, неограниченный
  пространственный контекст, включающий все галактики, звезды и другие
  астрономические объекты. Он характеризу...»
* mistral-7B RU TP-4: «Космос - это обширное пространство, заполненное
  звездами, планетами и галактиками. Он начинается от Земли и
  простирается до границы извест...»
* qwen3-4B RU TP-4: «<think>Хорошо, пользователь спрашивает, что такое
  космос, и хочет короткий ответ. Начну с того, что космос — это
  пространство за пределами Земли. Нужно упомянуть звезды, планеты,
  галактики и другие объекты. Важно подчеркнуть, что это огромное
  пространство с разными явлениями»
* gemma3-4B RU SP: «Космос – это всё, что существует: пространство,
  время, материя и энергия. Это огромнейшая пустота, в которой
  находятся звезды, планеты, галактики и всё остальное. Вот основные
  моменты: *»

50+ commits в одной сессии. Final commit `9b4b954` на `origin/main`.
...
* `test_rope_neox_fix.sh`, `test_rope_neox_tp4.sh`,
  ...
  `test_gemma3_tp4.sh`, `test_phi3_bisect_longrope.sh`,
  `test_phi3_bisect_quant.sh`.

### Bisect helpers (env flags)
* `PT_FORCE_ALL_GLOBAL=1` — force gemma3 все слои global rope (skip SWA).
* `PT_NO_LONGROPE=1` — disable phi3 rope_factors_short/long.
* `PT_NO_QUANT_GEMV=1` — force FP fallback (для phi3 Q5_K bisect).

### Plan tasks closed (sessions add-on)
* #59 try/catch в JSON parser (b675166)
* #60 CPU fast path для /api/chat (69b455e)
* #61 CI badges зелёные (ae881fd)
* #62 CITATION + FUNDING + CoC + PR template (39977f6)

Commits: `b144db2`..`0846cc8` (~40 commits в одной сессии).

### Финальный итог (2026-05-03 PM)

**Работают на русском + English:**
* mistral-7B (NORM RoPE) — без регрессии
* qwen2.5-7B (NEOX + chat template fix)
* qwen3-1.7B (NEOX) — 17.1 tok/s TP-4
* qwen3-4B (NEOX) — 8.0 tok/s TP-4
* qwen3-8B (NEOX)
* gemma3-4B (NEOX + per-layer SWA + GeGLU + post_norm) — 4.6 tok/s SP

**Не работают:**
* qwen3-0.6B — capacity issue (не code fix-able)
* phi3.5-mini — `bom<unk>×60`. Bisect: PT_NO_LONGROPE даёт ТОТ ЖЕ output,
  значит rope_factors не виноваты. Подозрение: Q5_K kernel split layout
  или per-head attn_qkv permutation. Pending: PT_NO_QUANT_GEMV bisect
  (rebuild14 в ходу), затем activation dump vs llama.cpp.
...
9. **Agent 8 — public SOTA:** наши 5.3/6.1 = at/above public E2K ceiling.
   **DIMM population на ELBRUS_HOST не проверен!** Elbrus-16C: 77 GB/s с 8 DIMMs vs 18.65 с 2 DIMMs.
...
всё ещё живёт история". Verified, was correct:
  ...
  * Commit bodies still mentioned "<user> username"
  * Blobs 3395793b / 8abaee65 / 46a249a0 still held the deleted folder
  * `<user>` username alive in `examples/nmquad/profile_nmquad.cpp` +
...
Ran git-filter-repo 2.47.0 with three passes combined in one run:
  ...
  2. `--replace-text` with rules file covering: <user> → user,
     [researcher]/Pugachev/Konstantin → partner, BENCH_KONSTANTIN → BENCH_ELBRUS,
...
Post-scrub verification against remote:
  * `git log --format='%H %s' | grep ...surnames/<user>/BENCH_K/НТЦ/NTC` → empty
  * `git rev-list --all --objects | grep НТЦ|БЕСЕДЫ` → empty
  * `git grep -l '<user>' origin/main` → empty
  * `git grep -il '[researcher]|Pugachev|Konstantin A\.'` → empty
...
| # | Item | Commit | Result |
|---|------|--------|--------|
| 1 | DIMM check on ELBRUS_HOST | — | `sudo` required for dmidecode; confirmed 4-NUMA × 32 GB, 256 hugepages (512 MB) |
...
### Прогресс генерации (loss ↓ → связность ↑)

Валидация pir_infer.py на чекпоинте 500: CE на срезе = 7.76 ≈ train-loss
7.71 → forward корректен (мусорный forward дал бы ~9.68). Генерация:
- шаг 500 (loss 7.7): «его у его до в котором раз. 18 На а когда не как»
- шаг 2000 (loss 6.3): «Москва является город — город, на которой был…»,
  «Россия — это не было их в своей его…» — зачаточная грамматика,
  связки (которые/чтобы/для этого), но целостности нет + выдуманные слова.

### Reboot Эльбруса + resume fused-тренировки (2026-07-23)

На шаге ~2900 сервер перезагрузился (uptime 1 min при реконнекте) — все
процессы умерли. `loginctl enable-linger` спасает только от
SSH-disconnect, НЕ от reboot. Последний сохранённый чекпоинт — 2500.

Fused-путь resume не поддерживал (`--load` был только для не-fused .ptor
через `torch::load_state_dict`; fused делал лишь `init_random`). Добавил
чтение сырого float32-дампа all_params в fused + `--start_step` (смещение
LR-графика и нумерации чекпоинтов). Коммит `5319bec`.

Первый resume дал буйный transient: loss 6.1→8.5, gnorm 40+. Причина —
Adam moments (m,v) в чекпоинт не пишутся, сброшены в 0, а LR на резюме
сразу ~пиковый. Фикс — resume-warmup: линейный разгон LR первые
warmup_steps после `--start_step`. Коммит `0d7b89b`. После фикса resume
чистый: loss 6.1–6.5, gnorm 8–13. Старый прогон остановлен прицельно по
PID (не по имени — правило безопасности процессов).

### Полный аудит корректности PIR (2026-07-23, по запросу) — 4 фикса

Прошёл всю математику fused-пути: chain rule каждого оператора,
GEMM-соглашения (linear Y=X·Wᵀ, bwd_input dY·W, bwd_weight dYᵀ·X),
scan-BPTT (dh-рекурсия, dgate=dh·h[t-1]), двойной путь градиента через
сигмоиду (gating-mul + scan-gate·base_decay), реконструкция
pir_residual_p вычитанием norm_out (FP-шум ~1ulp — ок), rmsnorm bwd
формула (inv_rms·(g·w − x·dot), dot=Σg·w·x·inv_rms²/D — верна), Adam
(bias correction от step_count согласован со сбросом moments на resume),
клиппинг, датасет (сдвиг +1, границы, uint32→float точен до 2²⁴),
Local SGD. Архитектурно всё ВЕРНО. Но 4 бага в ядрах (коммит `8997090`):

1. **rmsnorm_fwd требовал D%6==0** («D must be divisible by 6», историч.
   D=768). Текущая модель D=256 (256%6=4): на d=252 читал/писал
   xi[252..257] — inv_rms от 258 элементов (смещение ~0.4%), запись 2
   элементов в соседнюю строку (на границах OMP-чанков порча выживает),
   OOB-чтение weight[256..257], запись за буфер на последней строке.
   Фикс: 6-wide + хвост. Это объясняет расхождение numpy CE 7.76 vs
   train 7.71 — numpy-forward честный.
2. **rmsnorm_bwd in-place → dnorm2_w неверен.** norm2 bwd вызывается с
   grad==dx; внутри цикла dxi[d]=... перезаписывал gi[d], а следующий
   оператор dw_local[d] += gi[d]·... читал уже НОВОЕ значение → градиент
   norm2_w всех слоёв считался от преобразованного grad. Детерминированно
   неверно на всех платформах, затрагивал и исторический 342M (D=768).
   Фикс: dw до записи dxi — in-place теперь легален.
3. **FFN mul_bwd db==grad==dh** — нарушение __restrict (латентный UB на
   LCC/VLIW; спасал только register-dataflow). db → la.ffn_gated.
4. **CE без клампа target** — токен ≥ V (битый файл/неверный
   --vocab_size) = запись за границу dlogits. Кламп как в embed_forward.

Тренировка перезапущена на исправленном бинарнике с последнего
чекпоинта (веса валидны — это реальные float, «шум» был только в
вычислениях).

### EML не линковался: тренировка шла на сериальном TudaBLAS (~10× медленнее)

Вопрос юзера «все ли процессоры параллельно?» → mpstat: внутри каждого
NUMA-узла молотит 1 ядро из 8 (loadavg 4.4 вместо ~32). Раскопка:

1. Привязка процессов идеальна (taskset: 0-7/8-15/16-23/24-31), но
   fwd-время не зависело от OMP_NUM_THREADS (8 vs 32 — одинаково) и
   batch 2→8 масштабировался линейно по времени → GEMM сериальный.
2. TudaBLAS не имеет ни одного `omp parallel` в GEMM — сериальный по
   дизайну (фолбэк). ldd бинарника: **libeml нет вообще**, хотя
   CMakeCache говорил PT_USE_EML_BLAS=ON.
3. Root cause: CMake `try_run` проба EML при каком-то давнем configure
   зафейлилась и **закэшировалась**; `set(PT_USE_EML_BLAS OFF)` тихо
   перекрывал опцию → define не применялся → hot::sgemm_nt падал в
   `tuda::blas::sgemm_nt`. Косвенное подтверждение: BPE-segfault был
   именно в TudaBLAS PackBuffers — GEMM реально шёл через Tuda.
4. Микробенч EML_MT (cc -leml_mt, OMP=8, node-local): 2048³ = 240
   GFLOPS; наши формы: 512×256×256 = 126, 2048×256×256 = 152,
   512×16000×256 (lm_head) = 161 GFLOPS. Против ~5 GFLOPS Tuda-serial.

Фикс: `cmake -U '_PT_EML*' -DPT_EML_RUNTIME_OK=ON` (штатный override в
CMakeLists) + rebuild. ldd: libeml_mt.so ✓. Замер шага (contended node):
**fwd 1820→285мс (6.4×), bwd 4360→190мс (23×), шаг ~6.4с→0.62с (~10×)**.
per-proc ~78→~800 tok/s, 4-проц суммарно ~300→~3000 tok/s (~250 GFLOPS
эффективных = уровень PyTorch на 8-16-ядерном x86 десктопе).

Follow-up (не сделано): grad-clip петля в train_step сериальная
(~100мс/шаг = теперь ~16% шага) — окупится OMP-редукцией.
[UPD той же сессии: сделано, `2e399d9` — OMP-редукция в double.]

### arXiv:2607.14885 (Popovich, «Contraction vs Recurrence») → stateful декод

По запросу юзера прочитал статью + скачал ancillary-код (4 py-файла).
Суть: предсказание детерминированной динамики — рекуррентные методы
(метод аналогов, ждать возврата в ε-окрестность, стоимость ε^(-d)) vs
contraction/observer-методы (сходящийся оценщик состояния, log(1/ε)
шагов). Экспоненциальное разделение (на Лоренце разрыв ~1e9 при ε=1e-6).

Применение к PromeTorch — прямое попадание в PIR-генерацию:
скан h_t=a⊙h_{t-1}+u_t (a<1) — контракция, состояние = сжатая память
префикса. А генерация работала «методом аналогов»: полный перепрогон
контекста на каждый токен (O(T)/токен). Переведена на stateful:
состояние [L][NP][D] между токенами → O(1)/токен (`ce3fcce`).
Точность: max|Δlogits| = 0.00e+00 (бит-в-бит vs полный форвард).
Скорость: 1.3→8.9 tok/s (7×) уже на контексте 64 (numpy, E8C2);
рост линейный по длине контекста. Бонус: контекст не обрезается
block_size. Реализовано в pir_infer.py (default, --full-rescan/
--check-stateful) и fused_trainer.h::generate_text (C++).

Не применимо (честно): якорное подпространство (rank-r итерации одной
матрицы — таких путей у нас нет, granica-тест 1c статьи отверг бы);
гейт детерминизма через суррогаты — диагностика временных рядов, не perf.

### ТРЕНИРОВКА ЗАВЕРШЕНА: 10000 шагов, финальный loss 5.59 (2026-07-23)

Полный прогон BPE 16k русской LM (13.5M параметров) на E8C2 завершён.
Траектория: 9.68 (init) → 7.71 (500) → 6.32 (2000) → 5.89 (5000) →
**5.59 (10000, ppl 267)**. Прогон пережил: reboot сервера (resume с
2500), рестарт на исправленных ядрах (3500, аудит-фиксы), рестарт на
EML (5000, 14× ускорение). Последние 5000 шагов на EML — ~35 минут
против ~9 часов, которые заняли бы на сериальном TudaBLAS.

Финальная генерация (stateful, ckpt 10000): «Россия — это… В свою
очередь, „Калаге" — российский политик, член…», «Пролебат — российский
административный центр района», «В 1998 году в УСО…» — устойчивые
википедийные конструкции с согласованием; имена собственные выдуманные
(норма для 13.5M/16.7M токенов). Это LM продолжения текста; для
chat-ассистента нужен instruct-датасет вторым этапом.

### МЕГАПРОГОН: 50 ток/парам = 675M токенов (2026-07-23, задача юзера)

Цель юзера: loss ≤1.8, бюджет 50 токенов на параметр, всё русское.
Данные: russian_mega.txt (2.0GB) перетокенизирован BPE-16k 8-way
параллельно (батч-encode списком строк — 30× быстрее encode 4МБ-строкой,
весь корпус ~5 мин) → **russian_mega_16k.tokens = 440.2M токенов**.
wiki_ru_clean.txt проверен = дубль mega (cmp 100МБ + строки середины);
остальные russian_*.txt на сервере — мелкие подмножества mega. Старый
russian_mega.tokens (1.4GB) не годится — кодирован старым 100k-словарём.

Бенч BT-скейлинга (EML): batch8×block512 = batch16×block256 = 1606
tok/s/proc (per-token одинаково, compute-bound). Выбор: 8×512 (длиннее
контекст). Запуск: 4×NUMA, 41200 шагов × 16384 ток = 675.2M, lr 6e-4,
grad_accum 10, ckpt/1000, ETA ~29ч. Старт чистый: loss 9.676=ln(16000),
шаг 2.56с.

Честное ожидание по ёмкости: 13.5M на вики-BPE-16k выйдет на плато
~4.3-4.8 (даже GPT-2 124M даёт ~3.0-3.3) — цель 1.8 в этой шкале
недостижима без упрощения распределения данных (TinyStories-класс;
русского аналога в природе нет — проверено HF) или роста модели.
Сказано юзеру, решение — прогон по его формуле.

**ФИНИШ (2026-07-24):** «Fused training complete!» на шаге 41200, финал
loss 2.95, ppl 19.1. Кривая: 9.68→4.44(1k)→3.52(5k)→3.13(20k)→2.95(41k),
без плато. Пережил 2 reboot сервера (resume). ~5750 tok/s суммарно на EML
все ~30ч. Первая PIR-модель, обученная на Эльбрусе с нуля. Плато-прогноз
4.3-4.8 побит (данных 26× больше + контекст 512): реально 2.95.

### Бенчмарк базовой модели (2026-07-24, pir_bench.py, `f0cf455`)

Zero-shot Russian SuperGLUE через log-likelihood (модель выбирает
вероятнейший вариант) + held-out ppl на свежих русских текстах (пассажи
DaNetQA — вне претрейна). ~7с/forward в numpy (Python-скан) — DaNetQA
capped 200, контекст обрезан до 220 ток.

| Метрика | PIR-13.5M | baseline |
|---------|-----------|----------|
| held-out ppl | **58.8** | train 19 |
| PARus | 51.0% | chance 50% |
| TERRa | 51.5% | chance 50% |
| DaNetQA | 55.5% | majority 55% |

Вывод честный: язык выучен (ppl 59 на незнакомом тексте), commonsense =
chance (13.5M не хватает ёмкости для рассуждения; DaNetQA угадывает
мажоритарный класс). Норма для такого размера — RSG «решается» от ~1.5B.
Показательна перплексия, не RSG-accuracy. Follow-up: numpy-скоринг
медленный, для полного RSG нужен C++/батч-скоринг.

### Спекулятивный декод: механика работает, нужен дистилл-draft (`d68bb94`)

Draft D=64/L2 (~5% FLOP) обучен на russian_16k (loss 5.28@4600 —
лучше target 5.59: обе модели data-limited на 16.7M токенов!).
Реализация: draft предлагает K, target верифицирует батч-прогоном из
состояния скана, приёмка префикса + свой токен; жадный выход бит-в-бит
= обычному ✓. Замер: acc_rate 0.26 (draft не дистиллирован — top-1
расходятся), ток/раунд 2.03; в numpy МЕДЛЕННЕЕ (4.1 vs 7.9 tok/s,
python-overhead). План: после мегапрогона дистиллировать draft от
