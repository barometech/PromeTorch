...
int main()
{
    try
    {
        ...
        if (!out) throw std::runtime_error("cannot open linear_model_params.txt for writing");
        ...
        std::cout << "PromeTorch linear model params save smoke OK" << std::endl;
    ...
    {
        std::cerr << "PromeTorch linear model params save smoke FAILED: " << e.what() << std::endl;
        return 1;
    }
}
