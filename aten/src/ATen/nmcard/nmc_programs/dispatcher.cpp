// Unified Kernel Dispatcher for NMC4
// Single program that handles all operations via op code
// Load once, execute many - no reload needed between ops
//
...
#define OP_GATE_MUL     13  // a * silu(b)
#define OP_MUL_SCALAR   14
#define OP_GELU         15
#define OP_LAYERNORM    16

// Pre-loaded weight operations (weights stored as Q16.16 in DDR)
#define OP_MATMUL_DDR   20  // matmul with pre-loaded Q16.16 weights
#define OP_RMSNORM_DDR  21  // rmsnorm with pre-loaded Q16.16 gamma

// NEW: Exit command for graceful shutdown
#define OP_EXIT         255

// ============================================================
// Memory layout (32 words = 128 bytes command block)
// ============================================================
// [0]  = op (operation code, 0 = idle)
// [1]  = arg0 (varies by op)
// [2]  = arg1
// [3]  = arg2
// [4]  = arg3
...
// [31] = watchdog counter (increments in main loop)
...
// ============================================================
// MatMul DDR: C[M,N] = A_float[M,K] @ B_q16[K,N]
// A is float (activation, transferred each call)
// B is Q16.16 (pre-loaded in DDR, NO conversion needed!)
// args: [M, K, N, addr_A, addr_B_q16, addr_C, addr_tmp]
// addr_tmp: temp buffer for converted A row (size >= K)
// ============================================================
void op_matmul_ddr() {
    unsigned int M = mem[1];
    unsigned int K = mem[2];
    unsigned int N = mem[3];
    unsigned int* A = (unsigned int*)mem[4];     // float, activation
    fixed32* B = (fixed32*)mem[5];               // Q16.16, pre-loaded
    unsigned int* C = (unsigned int*)mem[6];      // float, output
    fixed32* a_buf = (fixed32*)mem[7];            // temp buffer for A row

    unsigned int a_off = 0;
    unsigned int c_off = 0;

    for (unsigned int i = 0; i < M; i++) {
        // Convert A row from float to Q16.16 once per row
        for (unsigned int k = 0; k < K; k++) {
...
// ============================================================
// RMSNorm DDR: same as rmsnorm but gamma is pre-loaded Q16.16
...
void op_rmsnorm_ddr() {
    ...
    unsigned int* input = (unsigned int*)mem[3];
    unsigned int* output = (unsigned int*)mem[4];
    fixed32* gamma = (fixed32*)mem[5];  // Q16.16, pre-loaded
    fixed32 eps = 1;
    ...
    for (unsigned int b = 0; b < batch; b++) {
        ...
        for (unsigned int i = 0; i < hidden; i++) {
            fixed32 xi = float_to_fixed(x[i]);
            fixed32 g = gamma[i];  // Already Q16.16!
            y[i] = fixed_to_float(mul_fixed(mul_fixed(xi, inv_rms), g));
        }
    }
}
...
int main() {
    ...
    while (1) {
        // Update watchdog so host knows we're alive
        watchdog++;
        ...
        // NEW: Exit command for graceful shutdown
        if (op == OP_EXIT) {
            mem[STATUS_ADDR] = 1;  // acknowledge
            mem[0] = OP_NOP;
            break;  // Exit the loop and return from main
        }
