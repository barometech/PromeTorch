#pragma once
// ============================================================================
// MicroKernel_4x4.h — E2K VLIW-friendly GEMM micro-kernel for Elbrus
// ============================================================================
// ВНИМАНИЕ: НЕ ИСПОЛЬЗУЕТСЯ. Историческая первая версия, рассчитанная на 4 FMA.
// Реально E8C2 даёт 6 арифметико-логических каналов, поэтому TudaBLAS.h
// подключает MicroKernel_6x6.h (36 аккумуляторов), а kE2K в TudaConfig.h
// задан как {MR=6, NR=6, ...}. Этот файл оставлен для истории и в
// dispatch_microkernel не входит. Не путать с kernels/nmc4/MicroKernel_4x4.h —
// то ядро для NM Card и оно действительно используется.
// ============================================================================
// C[4×4] += A[4×K] × B[K×4]
// Pure C — LCC compiler handles VLIW scheduling and packed FMA.
// 16 independent FMA operations per K-step → fills 4 FMA units.
// K-loop unrolled 4× for software pipelining.
// Uses __restrict and array indexing for LCC alias analysis.
// ============================================================================

#include <cstdint>

namespace at {
namespace native {
namespace tuda {
namespace kernels {

static void microkernel_4x4_e2k(
    int64_t K,
    const float* __restrict A,
    const float* __restrict B,
    float* __restrict C,
    int64_t ldc,
    float alpha,
    float beta
) {
    // 16 accumulators — all independent, LCC keeps them in registers
    float c00=0,c01=0,c02=0,c03=0;
    float c10=0,c11=0,c12=0,c13=0;
    float c20=0,c21=0,c22=0,c23=0;
    float c30=0,c31=0,c32=0,c33=0;

    // Unroll 4× to give LCC a large loop body for software pipelining
    int64_t p = 0;
    for (; p + 4 <= K; p += 4) {
        for (int64_t pp = 0; pp < 4; ++pp) {
            float a0 = A[(p+pp)*4+0], a1 = A[(p+pp)*4+1];
            float a2 = A[(p+pp)*4+2], a3 = A[(p+pp)*4+3];
            float b0 = B[(p+pp)*4+0], b1 = B[(p+pp)*4+1];
            float b2 = B[(p+pp)*4+2], b3 = B[(p+pp)*4+3];
            c00 += a0*b0; c01 += a0*b1; c02 += a0*b2; c03 += a0*b3;
            c10 += a1*b0; c11 += a1*b1; c12 += a1*b2; c13 += a1*b3;
            c20 += a2*b0; c21 += a2*b1; c22 += a2*b2; c23 += a2*b3;
            c30 += a3*b0; c31 += a3*b1; c32 += a3*b2; c33 += a3*b3;
        }
    }
    for (; p < K; ++p) {
        float a0 = A[p*4+0], a1 = A[p*4+1], a2 = A[p*4+2], a3 = A[p*4+3];
        float b0 = B[p*4+0], b1 = B[p*4+1], b2 = B[p*4+2], b3 = B[p*4+3];
        c00+=a0*b0; c01+=a0*b1; c02+=a0*b2; c03+=a0*b3;
        c10+=a1*b0; c11+=a1*b1; c12+=a1*b2; c13+=a1*b3;
        c20+=a2*b0; c21+=a2*b1; c22+=a2*b2; c23+=a2*b3;
        c30+=a3*b0; c31+=a3*b1; c32+=a3*b2; c33+=a3*b3;
    }

    // Store with alpha/beta
    if (beta == 0.0f) {
        C[0*ldc+0]=alpha*c00; C[0*ldc+1]=alpha*c01; C[0*ldc+2]=alpha*c02; C[0*ldc+3]=alpha*c03;
        C[1*ldc+0]=alpha*c10; C[1*ldc+1]=alpha*c11; C[1*ldc+2]=alpha*c12; C[1*ldc+3]=alpha*c13;
        C[2*ldc+0]=alpha*c20; C[2*ldc+1]=alpha*c21; C[2*ldc+2]=alpha*c22; C[2*ldc+3]=alpha*c23;
        C[3*ldc+0]=alpha*c30; C[3*ldc+1]=alpha*c31; C[3*ldc+2]=alpha*c32; C[3*ldc+3]=alpha*c33;
    } else {
        C[0*ldc+0]=alpha*c00+beta*C[0*ldc+0]; C[0*ldc+1]=alpha*c01+beta*C[0*ldc+1];
        C[0*ldc+2]=alpha*c02+beta*C[0*ldc+2]; C[0*ldc+3]=alpha*c03+beta*C[0*ldc+3];
        C[1*ldc+0]=alpha*c10+beta*C[1*ldc+0]; C[1*ldc+1]=alpha*c11+beta*C[1*ldc+1];
        C[1*ldc+2]=alpha*c12+beta*C[1*ldc+2]; C[1*ldc+3]=alpha*c13+beta*C[1*ldc+3];
        C[2*ldc+0]=alpha*c20+beta*C[2*ldc+0]; C[2*ldc+1]=alpha*c21+beta*C[2*ldc+1];
        C[2*ldc+2]=alpha*c22+beta*C[2*ldc+2]; C[2*ldc+3]=alpha*c23+beta*C[2*ldc+3];
        C[3*ldc+0]=alpha*c30+beta*C[3*ldc+0]; C[3*ldc+1]=alpha*c31+beta*C[3*ldc+1];
        C[3*ldc+2]=alpha*c32+beta*C[3*ldc+2]; C[3*ldc+3]=alpha*c33+beta*C[3*ldc+3];
    }
}

} // namespace kernels
} // namespace tuda
} // namespace native
} // namespace at
