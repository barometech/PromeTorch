...
void test_linalg() {
    ...
    // Verify L @ U ≈ P @ M (PA = LU)
    auto LU = at::native::mm(lu_res.L, lu_res.U);
    auto PM = at::native::mm(lu_res.P, M);
    double lu_err = max_abs_diff(LU, PM);
    CHECK(lu_err < 1e-4, "LU ≈ P@M (err=" + std::to_string(lu_err) + ")");
...
void test_fft() {
    ...
    auto fft_result = at::native::fft(signal, -1, 0);   // dim=0 is the data dim
    auto ifft_result = at::native::ifft(fft_result, -1, 0);
...
void test_conv_transpose() {
    ...
    torch::nn::ConvTranspose2d conv_t(1, 1, 2, 1, 0);  // in=1, out=1, kernel=2, stride=1, pad=0

    // Set all weights to 1.0
    auto params = conv_t.parameters();
    {
        auto& wd = params[0]->data();
        auto wp = wd.mutable_data_ptr<float>();
        for (int i = 0; i < (int)wd.numel(); i++) wp[i] = 1.0f;
    }
    ...
    double sum = 0;
    for (int64_t i = 0; i < output.numel(); i++) sum += std::abs(out_ptr[i]);
...
void test_quantization() {
    ...
    // Use observer to compute proper scale/zero_point
    torch::quantization::MinMaxObserver obs_for_quant;
    obs_for_quant.forward(x);
    auto [scale, zero_point] = obs_for_quant.calculate_qparams();
    ...
    CHECK(q_err < scale * 1.5, "dequant(quant(x)) ≈ x within 1.5*scale (err=" + std::to_string(q_err) + ", scale=" + std::to_string(scale) + ")");
    ...
    CHECK_CLOSE(qt.q_scale(), scale, 1e-10, "Quantized tensor scale preserved");
    CHECK(qt.q_zero_point() == zero_point, "Quantized tensor zero_point preserved");
}
