// ============================================================================
// MNIST Training Example - CNN (Convolutional Neural Network)
// ============================================================================
...
public:
    MNISTConvNet() : Module("MNISTConvNet") {
        // Conv layers
        // Input: [B, 1, 28, 28]
        conv1 = std::make_shared<Conv2d>(1, 32, 3, 1, 1);   // -> [B, 32, 28, 28]
        conv2 = std::make_shared<Conv2d>(32, 64, 3, 1, 1);  // -> [B, 64, 28, 28]

        // After max pooling 2x2: [B, 64, 14, 14]
        // After second pooling: [B, 64, 7, 7] = 3136
        ...
        dropout = std::make_shared<Dropout>(0.25f);
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "CNN Model created (Conv32->Conv64->FC128->10)" << std::endl;
    ...
    for (int64_t epoch = 1; epoch <= epochs; ++epoch) {
        ...
        std::cout << "  Test Acc: " << (100.0f * test_correct / n_test) << "%" << std::endl;
    }

    std::cout << "\n=== CNN Training Complete ===" << std::endl;
    return 0;
}
