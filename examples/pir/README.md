...
батчах, веса усредняются через файлы в `/tmp` каждые `GRAD_ACCUM` шагов.
...
GFLOPS = 80% расчётного пика 2304 на больших GEMM).
...
**Stateful-декод (по умолчанию).** Генерация несёт состояние скана между
токенами (контракционный взгляд, [arXiv:2607.14885](https://arxiv.org/abs/2607.14885)):
O(1) на токен вместо перепрогона всего контекста, логиты бит-в-бит те же
(`--check-stateful` сверяет), контекст не ограничен block_size. Замер на
E8C2: 1.3 → 8.9 tok/s (7×) уже на контексте 64; выигрыш растёт с длиной.
Старый путь: `--full-rescan`.

## SFT: агентские действия + факты (фаза 2)

`build_sft_corpus.py` собирает русский SFT-корпус из 6 HF-источников
(~1.19M сэмплов / ~260M токенов): инструкции (ZeroAgency score≥7,
GrandMaster-PRO-MAX, saiga, russian_instructions), **агентские цепочки**
glaive-function-calling-ru (`### Инструменты → ### Задача → ### Действие
{json} → ### Результат → ### Ответ`) и **факты** sberquad (`### Факты →
### Вопрос → ### Ответ`, контекст обрезается окном вокруг ответа).
Шаблоны текстовые — в BPE-16k нет чат-токенов. Дальше стандартный
конвейер: prepare_bpe.py --skip-train (токенизация тем же словарём) →
train_local_sgd.sh с `--load <претрейн-ckpt> --start_step 0` и LR ~1e-4.
