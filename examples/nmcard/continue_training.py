"""Continue 109K transformer training from saved weights. Batch=4, target loss 1.6."""
...
with open(f'{sd}/config.json') as f: cfg=json.load(f)
V=cfg['V'];D=cfg['D'];H=cfg['H'];F=cfg['F'];T=cfg['T'];L=cfg['L'];HD=cfg['HD']

with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'..','..','data','tiny_shakespeare.txt'),'r') as f:
    text=f.read()
chars=cfg['chars'];ch2idx={c:i for i,c in enumerate(chars)};idx2ch={i:c for i,c in enumerate(chars)}
data=np.array([ch2idx[c] for c in text],dtype=np.int32)

embed=np.load(f'{sd}/embed.npy');pos=np.load(f'{sd}/pos.npy')
gf=np.load(f'{sd}/gf.npy');Wh=np.load(f'{sd}/Wh.npy');bh=np.load(f'{sd}/bh.npy')
layers=[]
for i in range(L):
    l={}
    for k in ['g1','Wq','Wk','Wv','Wo','g2','W1','b1','W2','b2']:
        l[k]=np.load(f'{sd}/l{i}_{k}.npy')
    layers.append(l)
print(f'Loaded: {cfg["params"]:,} params, loss={cfg["loss"]:.3f}, {cfg["steps"]} steps done')
...
card_mm_count=0
def card_mm(A,B):
    global card_mm_count; card_mm_count+=1
...
# Training with batch=4 gradient accumulation
print(f'\n=== CONTINUE TRAINING (batch=4, target 1.6) ===')
lr=0.001; best=cfg['loss']; step=0; start=time.time(); BATCH=4
...
while best > 1.6 and step < 20000:
    ...
    for l in layers:
        for k in ['Wq','Wk','Wv','Wo','W1','b1','W2','b2']:
            acc_grads[id(l[k])] = np.zeros_like(l[k])
    acc_grads['Wh'] = np.zeros_like(Wh)
    acc_grads['bh'] = np.zeros_like(bh)
    acc_grads['pos'] = np.zeros_like(pos)
    acc_grads['embed'] = {}
    batch_loss = 0
    ...
    for bi in range(BATCH):
        ...
        for li,l in enumerate(layers):
            xn,r1=rn(x,l['g1'])
            Q=card_mm(xn,l['Wq']);K_=card_mm(xn,l['Wk']);V_=card_mm(xn,l['Wv'])
            ...
            pr=card_mm(ao,l['Wo']);xr=x+pr
            xn2,r2=rn(xr,l['g2'])
            h=card_mm(xn2,l['W1'])+l['b1'];a=np.maximum(h,0)
            o=card_mm(a,l['W2'])+l['b2'];xn_=xr+o
            cache.append((x,xn,r1,Q,K_,V_,at,ao,xr,xn2,r2,h,a));x=xn_

        xf,rf=rn(x,gf)
        logits=card_mm(xf,Wh)+bh
        probs=sm(logits)
        ...
        acc_grads['Wh']+=xf.T@dl;acc_grads['bh']+=dl.sum(0)
        dx=rn_bwd(dl@Wh.T,x,gf,rf)

        for li in range(L-1,-1,-1):
            l=layers[li];x_in,xn,r1,Q,K_,V_,at,ao,xr,xn2,r2,h,a=cache[li]
            do=dx;acc_grads[id(l['W2'])]+=a.T@do;acc_grads[id(l['b2'])]+=do.sum(0)
            da=do@l['W2'].T*(h>0)
            acc_grads[id(l['W1'])]+=xn2.T@da;acc_grads[id(l['b1'])]+=da.sum(0)
            dxn2=da@l['W1'].T;dx_r=dx+rn_bwd(dxn2,xr,l['g2'],r2)
            dpr=dx_r;acc_grads[id(l['Wo'])]+=ao.T@dpr;dao=dpr@l['Wo'].T
            ...
            acc_grads[id(l['Wq'])]+=xn.T@dQ;acc_grads[id(l['Wk'])]+=xn.T@dK;acc_grads[id(l['Wv'])]+=xn.T@dV_
            dxn_attn=dQ@l['Wq'].T+dK@l['Wk'].T+dV_@l['Wv'].T
            dx=dx_r+rn_bwd(dxn_attn,x_in,l['g1'],r1)

        acc_grads['pos']+=0.1*dx
        # Vectorized embedding gradient accumulation via numpy advanced indexing
        if 'embed_full' not in acc_grads:
            acc_grads['embed_full'] = np.zeros((V, D), np.float32)
        np.add.at(acc_grads['embed_full'], tok[:Tl], 0.1 * dx[:Tl])

    batch_loss/=BATCH
    if batch_loss<best:best=batch_loss

    # Apply accumulated gradients / BATCH
    Wh-=lr*acc_grads['Wh']/BATCH;bh-=lr*acc_grads['bh']/BATCH
    pos-=lr*acc_grads['pos']/BATCH
    embed-=lr*acc_grads['embed_full']/BATCH
    for l in layers:
        for k in ['Wq','Wk','Wv','Wo','W1','b1','W2','b2']:
            l[k]-=lr*acc_grads[id(l[k])]/BATCH

    if step==5000:lr=0.0005
    if step==10000:lr=0.0002
...
with open(f'{sd}/config.json','w') as f:
    json.dump({'V':V,'D':D,'H':H,'F':F,'T':T,'L':L,'HD':HD,'params':cfg['params'],
               'loss':float(best),'card_matmuls':cfg['card_matmuls']+card_mm_count,
               'steps':cfg['steps']+step,'time':cfg['time']+el,'chars':''.join(chars)},f,indent=2)
print(f'Saved: {sd}/')
