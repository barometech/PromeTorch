...
- **Critical insight from ICL article:** engineering sample tested had "partially disabled cache" — production 16C should be faster. Also tested with only 2×32GB DIMMs: dropped to **18.65 GB/s Triad**. That's a HUGE bandwidth delta (4×) from memory population. **Check how many DIMMs are installed on ELBRUS_HOST.** If fewer than 8, we're bandwidth-starved by hardware, not software.

**Action item for infrastructure:** SSH into ELBRUS_HOST and run `dmidecode -t memory | grep -E "Size|Speed"` to verify DIMM population. If <8 DIMMs, document this as a physical ceiling, not an optimization failure.
...
- **Implication for our ELBRUS_HOST benchmark: if we have NATIVE access to ELBRUS_HOST, our numbers are more credible than the 2023 public reference. Document this in any comparisons.**
...
### For Round-2 agents
- **Agent 2 (memory bandwidth)**: verify DIMM population on ELBRUS_HOST (ssh + dmidecode). 77 GB/s assumes 8 DIMMs on 16C. On 8C2 equivalent, 4 DIMMs fully populated should give ~30 GB/s measured. If we see <20 GB/s STREAM, bandwidth is THE bottleneck and no software optimization will move us past ~8 tok/s on that chip alone.
