...
void init_tensor_bindings(py::module& m) {
    ...
    // tensor(data, dtype=..., device=..., requires_grad=...) — принимает:
    //   * py::array (numpy)            → прямой путь
    //   * py::list / py::tuple         → конверт через np.asarray
    //   * скаляр (int/float/bool)      → np.asarray делает 0-D
    //   * existing at::Tensor          → копия + перевод dtype/device
    //
