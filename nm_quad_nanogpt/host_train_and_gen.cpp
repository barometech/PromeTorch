/* ============================================================================
 * host_train_and_gen — full pipeline:
 *   1. Upload TinyStories to all 16 NMC4 cores' EMI
 *   2. Launch parallel training (nanogpt_train_v3 saves weights to EMI)
 *   3. Read trained weights from each core
 *   4. Pick the best (lowest loss) core's weights
 *   5. Run inference on the host: generate ~200 bytes from a seed prompt
 *
 * Generation is greedy (argmax) for determinism.
 * ============================================================================ */
...
/* NMC native word addresses for nanogpt_train_v3.abs.
 * Update via `nm nmc_part.abs | grep -E 'training_data|core_loss_out|saved_weights'` */
/* From `nm nmc_part.abs` after building v3 with per-core saved_weights_pool: */
static PL_Addr ADDR_TRAINING_DATA = 0x020603eaUL;
static PL_Addr ADDR_CORE_LOSS_OUT = 0x020603e8UL;
static PL_Addr ADDR_SAVED_WEIGHTS = 0x0201b7e8UL;   /* pool start, +slot*N_WEIGHTS for per-core */

static const char *NMC_PART  = "./nmc_part.abs";
static const char *DATA_PATH = "/home/<user>/nanogpt/data/tinystories_valid.txt";

/* ---- generation model in plain FP32 (one copy on the host) ---- */
struct Model {
    float Wtok[VOCAB][D];
    float Wpos[T][D];
    float Wqkv[D][3 * D];
...
/* Forward pass on host for one position — returns probs[t-1] over VOCAB. */
...
int main(int argc, char *argv[]) {
    ...
    /* ---- Load TinyStories into host buffer ---- */
        ...
        for (int c = 0; c < N_CLUSTERS; ++c)
            for (int n = 0; n < N_CORES; ++n) {
                ...
                if (status == PROGRAM_FINISHED) {
                    PL_Word lw = 0;
                    PL_ReadMemBlock(access[c][n], &lw, ADDR_CORE_LOSS_OUT, 1);
    ...
    /* Find best core (lowest loss in "healthy" range).
     * Exclude obviously-degenerate cores (loss < 0.5 → overfit to shard or
     * NaN-recovered to degenerate state). Real TinyStories byte-level
     * 1-layer transformer plateaus around ln(8) ≈ 2.1 nats. */
    int best_c = -1, best_n = -1; float best_loss = 1e30f;
    for (int c = 0; c < N_CLUSTERS; ++c)
        for (int n = 0; n < N_CORES; ++n)
            if (done[c][n] && loss[c][n] > 0.5f && loss[c][n] < best_loss) {
                best_loss = loss[c][n]; best_c = c; best_n = n;
            }
    if (best_c < 0) { std::cerr << "ERROR: no successful core\n"; return 3; }
    std::cerr << "[host] Best core: chip=" << best_c << " core=" << best_n
              << " loss=" << best_loss << "\n";

    /* Read weights from best core's slot in saved_weights_pool. */
    int best_slot = best_c * 4 + best_n;
    PL_Addr addr_slot = ADDR_SAVED_WEIGHTS + (PL_Addr)best_slot * N_WEIGHTS;
    std::vector<PL_Word> wbuf(N_WEIGHTS);
    if (PL_ReadMemBlock(access[best_c][best_n], wbuf.data(),
                        addr_slot, N_WEIGHTS) != PL_OK) {
        std::cerr << "ERROR: ReadMemBlock saved_weights slot " << best_slot << " failed\n";
        return 4;
    }
    std::cerr << "[host] Read weights from slot=" << best_slot
              << " (chip=" << best_c << " core=" << best_n << ")\n";
    /* NMC float == 32-bit word, same bit pattern as host float */
    ...
    /* Save to disk for later use */
    {
        ...
        std::cerr << "[host] Saved trained_weights.bin (" << N_WEIGHTS * 4 << " bytes)\n";
    }
    ...
    /* ---- Generate text using the best weights ---- */
    Model m;
    load_from_flat(m, wflat.data());

    std::vector<int> ctx;
    for (size_t i = 0; i < std::strlen(prompt); ++i) ctx.push_back(((int)prompt[i]) & 0x7F);

    std::cout << "\n=== Generation (greedy, best core chip=" << best_c
              << " core=" << best_n << " loss=" << best_loss << ") ===\n";
    std::cout << prompt;
    std::cout.flush();
