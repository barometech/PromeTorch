/* ============================================================================
 * host_4chip_safe — runs default-mode nmc_part.abs on chip K core 0 for K=0..3.
 * Each chip's EMI is physically separate, so no race between chips even when
 * the binary uses the same logical EMI addresses.
 *
 * Within one cluster we use only ONE core → no within-cluster race either.
 *
 * 4 cores trained in parallel ~26 min wall, 4 independently trained models.
 * Host picks best, runs greedy inference.
 * ============================================================================ */
...
/* Addresses from nm of default-mode nmc_part.abs (built earlier): */
...
int main(int argc, char *argv[]) {
    ...
    /* One core per cluster: cluster K, core 0. */
    PL_Access  *access[N_CLUSTERS] = {};
    IO_Service *service[N_CLUSTERS] = {};
    bool        done[N_CLUSTERS] = {};
    float       loss[N_CLUSTERS] = {};

    auto t0 = std::chrono::steady_clock::now();
    int loaded = 0;
    for (int c = 0; c < N_CLUSTERS; ++c) {
        PL_CoreNo core; core.cluster_id = c; core.nm_id = 0;
        if (PL_GetAccess(board, &core, &access[c]) != PL_OK) {
            std::cerr << "WARN: chip=" << c << " GetAccess failed\n"; continue;
        }
        if (PL_LoadProgramFile(access[c], NMC_PART) != PL_OK) {
            std::cerr << "WARN: chip=" << c << " LoadProgramFile failed\n";
            PL_CloseAccess(access[c]); access[c] = nullptr; continue;
        }
        if (PL_WriteMemBlock(access[c], word_buf.data(),
                             ADDR_TRAINING_DATA, (DWORD)DATA_SIZE) != PL_OK) {
            std::cerr << "WARN: chip=" << c << " WriteMemBlock failed\n";
            PL_CloseAccess(access[c]); access[c] = nullptr; continue;
        }
        service[c] = IO_ServiceStart(NMC_PART, access[c], nullptr, nullptr, nullptr);
        ++loaded;
    }
    std::cerr << "[host] Loaded " << loaded << "/" << N_CLUSTERS << " chips; training core 0 of each...\n";
    ...
    while (done_count < loaded && wait_limit-- > 0)
        for (int c = 0; c < N_CLUSTERS; ++c) {
            ...
            if (status == PROGRAM_FINISHED) {
                ...
                loss[c] = l;
                std::cout << "[chip=" << c << "] loss=" << l << std::endl;
                done[c] = true; ++done_count;
            } else { done[c] = true; ++done_count; }
        }

    auto t1 = std::chrono::steady_clock::now();
    std::cerr << "[host] Done in " << std::chrono::duration<double>(t1 - t0).count() << " s\n";

    /* Best: lowest loss in healthy range */
    int best_c = -1; float best_loss = 1e30f;
    for (int c = 0; c < N_CLUSTERS; ++c)
        if (done[c] && loss[c] > 0.5f && loss[c] < 4.0f && loss[c] < best_loss) {
            best_loss = loss[c]; best_c = c;
        }
    if (best_c < 0) {
        for (int c = 0; c < N_CLUSTERS; ++c)
            if (done[c] && loss[c] > 0 && loss[c] < best_loss) { best_loss = loss[c]; best_c = c; }
    }
    std::cerr << "[host] Best: chip=" << best_c << " loss=" << best_loss << "\n";

    /* Read weights from chip best_c slot 0 (slot index = best_c*4+0 = best_c*4). */
    int slot = best_c * 4 + 0;
    PL_Addr addr_slot = ADDR_SAVED_WEIGHTS + (PL_Addr)slot * N_WEIGHTS;
    std::vector<PL_Word> wbuf(N_WEIGHTS);
    if (PL_ReadMemBlock(access[best_c], wbuf.data(), addr_slot, N_WEIGHTS) != PL_OK) {
    ...
    std::cerr << "[host] Saved trained_weights.bin from chip=" << best_c << "\n";
    ...
    std::cout << "\n=== Generation (greedy, chip=" << best_c << " loss=" << best_loss << ") ===\n" << prompt;
    std::cout.flush();
