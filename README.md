...
- [Coverage vs PyTorch](#coverage-vs-pytorch)
...
- [Известные ограничения (Known Limitations)](#known-limitations--честный-gap-от-pytorch)
- [**Быстрый старт**](#быстрый-старт)
  - [Build Matrix — что собирается под что](#build-matrix--что-собирается-под-что)
  - [Сборка под Linux / macOS / WSL](#сборка-linux--macos--wsl)
  - [Сборка под Windows MSVC + CUDA](#сборка-windows-msvc--cuda)
  - [**Сборка на Эльбрусе**](#сборка-на-эльбрусе-нативно) → также см. [docs/BUILD_ELBRUS.md](docs/BUILD_ELBRUS.md)
  - [Сборка с NM Card Mini](#сборка-с-nm-card-mini)
  - [Сборка для российских ОС (Docker)](#сборка-для-российских-ос-docker)
- [Обучение MNIST (C++)](#обучение-mnist-c)
- [PromeServe — Ollama-совместимый LLM-сервер](#promeserve--ollama-совместимый-llm-сервер)
- [Справочник API](#справочник-api-api-reference)
- [Архитектура](#архитектура)
- [Компоненты](#компоненты)
- [Бенчмарки CPU (vs PyTorch 2.10)](#бенчмарки-cpu-vs-pytorch-210)
- [Тестирование](#тестирование)
- [CMake опции](#cmake-опции)
...
1840 GFLOPS через node-local EML_MT — 80% расчётного пика E8C2 в 2304 GFLOPS
(6 каналов × 128 бит × 1.5 ГГц × 32 ядра); от округлённых «2 TFLOPS» из
материалов МЦСТ это 92%. На других задачах
...
> 🛠 **Полный гайд по сборке и запуску:** [docs/BUILD_ELBRUS.md](docs/BUILD_ELBRUS.md) · [BENCH_ELBRUS.md](BENCH_ELBRUS.md)
...
| qwen3-14B | OOM TP-4 / **1.5 SP** | 1.02 | — | **✅ CoT RU** SP |
| **deepseek-coder-7B** | OOM TP-4 / **3.0 SP** | — | — | **✅ Python code** SP (после `81a79bd` rope.scale_linear fix) |
...
> **²** gemma3 TP-4 FIXED 2026-05-03 (commit `0ba114a`, требует `PT_TP_GATHER=1`):
> `post_attention_norm` + `post_ffw_norm` применяются на full h_buf после
> output_proj/ffn_down. 6.7 tok/s, structured markdown сохранён:
> «Космос – это всё, что существует за пределами нашей Земли. Это огромная,
> бесконечная область, включающая в себя: * **Вселенная:** Все известные
> объекты, включая звезды, планеты, галактики...» &nbsp;
...
| EML GFLOPS | 330 | **1840 (80% пика 2304)** | 68 (generic BLAS) |
...
| NVIDIA | Turing+ (sm_75) | CUDA | cuBLAS wrapper + custom Q4_K GEMV | matmul через cuBLAS; 149 CUDA-ядер в 12 .cu файлах (см. STATS.json) |
...
### Build Matrix — что собирается под что

| Платформа | Backend | Скрипт | Время | Output dir | Гайд |
|---|---|---|---|---|---|
| Windows MSVC 2019 | CPU + AVX2 | `BUILD_CORRECT.bat` | ~10 мин | `build_correct/` | [docs/BUILD_WINDOWS.md](docs/BUILD_WINDOWS.md) |
| Windows MSVC 2019 | CUDA 12.4 + cuDNN | `scripts/build-cuda-windows.bat` | ~25 мин | `build_cuda124/` | [docs/BUILD_WINDOWS.md](docs/BUILD_WINDOWS.md) |
| Linux GCC | CPU + AVX2 | `scripts/build-astra.sh` (или вручную) | ~10 мин | `build_astra/` | этот файл |
| Linux + CUDA | A100 sm_80 | `Dockerfile.cuda` | ~30 мин | docker | этот файл |
| **Эльбрус Linux 8** | **E2K VLIW + EML + NUMA** | **`scripts/build-elbrus.sh`** | **~15-25 мин** | **`build_elbrus/`** | **[docs/BUILD_ELBRUS.md](docs/BUILD_ELBRUS.md)** |
| NM Card Mini | Q16.16 эмулятор | `build_nmcard.bat` | ~10 мин | `build_nmcard/` | [BENCH_NMCARD.md](BENCH_NMCARD.md) |
| Astra/ALT/RED Linux | Docker (CPU AVX2) | `docker/build-all.sh` | ~10 мин | docker images | этот файл |
| Baikal-M/S aarch64 | NEON cross | `scripts/build-baikal-cross.sh` | ~10 мин | `build_baikal/` | этот файл |
...
./scripts/run_tp_elbrus.sh ~/gguf_models/qwen3-4b-Q4_K_M.gguf "Привет"

# Single-process (для 8B+):
./scripts/run_1proc_elbrus.sh ~/gguf_models/qwen3-14b-Q4_K_M.gguf "..."
```

> 🛠 **Полный гайд:** [docs/BUILD_ELBRUS.md](docs/BUILD_ELBRUS.md) — toolchain, CMake флаги, env vars TP-4, troubleshooting, известные подводные камни (loginctl, EML SIGILL, PT_PIN_THREADS, PT_Q8_SOA).

> 📊 **Результаты:** PromeTorch TP-4 на Эльбрус-8С2 (4 чипа × 8 ядер):
> - qwen3-1.7B 17.1 tok/s · qwen3-4B 10.9 · mistral-7B 8.5 · gemma3-4B 6.7
> - Speedups vs llama.cpp 32t: **×6.3 / ×6.0 / ×4.9 / ×5.2**
...
- **Windows: `rc.exe не найден`** — запускать из Developer Command Prompt for VS 2019, не из обычного cmd. См. [docs/BUILD_WINDOWS.md](docs/BUILD_WINDOWS.md).
- **Windows: `nv/target` header missing** — Anaconda CUDA 12.9 несовместима. Поставить standalone CUDA Toolkit 12.4 от NVIDIA.
- **Linux: cuDNN 9 RNN compile fail** — guarded; используй cuDNN 8 либо `-DPT_USE_CUDNN=OFF`.
- **Эльбрус: `cblas_sgemm SIGILL`** — старая версия. Текущий код запрещает EML cblas из pthread. См. [docs/BUILD_ELBRUS.md](docs/BUILD_ELBRUS.md#подводные-камни).

### Запуск GGUF inference на Эльбрусе

- **TP-4 даёт 1.4 tok/s вместо 9-11** — `PT_PIN_THREADS=1` ломает NUMA. Снять переменную.
- **Скорость 7.7 вместо 11.4 на qwen3-4B** — забыл `PT_Q8_SOA=1`. Без флага legacy путь.
- **Процесс убивается при SSH disconnect** — `loginctl enable-linger $USER` после каждого reboot сервера.
- **`Conflicting policies` от numactl** — не комбинировать `--membind` с `--preferred`. Использовать только `--membind` (TP-4) или `--interleave=all` (single-process).

### CUDA inference на A100

- **OOM на larger models** — quant-only mode уже включён, увеличить `--max-tokens` лимит или взять модель поменьше.
- **gemma3:27b crash** — known issue с tied weights, см. JOURNAL.md.
