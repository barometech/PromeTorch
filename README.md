...
**PromeTorch License** — модифицированная BSD 3-Clause (базовая лицензия PyTorch) с одним
дополнительным условием. См. полный текст: [LICENSE](LICENSE).

### Коротко

✅ **Можно БЕСПЛАТНО и НА КОММЕРЦИЮ:**
- Обучать нейросети / модели с помощью PromeTorch — любые, включая коммерческие.
- Запускать inference на PromeTorch — в SaaS, API, on-prem, mobile, hardware-embedded.
- Продавать / лицензировать **модели, веса, pipeline'ы, приложения** которые используют
  PromeTorch как runtime. Твой продукт — твой доход.
- Делать consulting, fine-tuning, training-as-a-service — где PromeTorch это underlying
  фреймворк.
- Форкать, модифицировать, встраивать в свой software, publish derivative models.

❌ **НЕЛЬЗЯ:**
- Переименовать PromeTorch в "YourDL Framework 2.0" и продавать как свой собственный
  фреймворк/библиотеку/SDK.
- Делать "Commercial Edition" / "Enterprise Version" PromeTorch с paywall на core
  functionality.
- Взимать license fees, subscription fees, или per-seat fees за **доступ к самому
  фреймворку**.
- Продавать исходный код PromeTorch как продукт.

### В двух словах

**Фреймворк свободен и открыт. Твои модели и продукты на его базе — твои, хоть в миллиард.
Сам фреймворк перепродавать — нельзя, он остаётся общим.**

Это делает PromeTorch **source-available** лицензией (а не строго OSI-compliant "open
source"). Все обычные бенефиты open-source сохраняются — просто нельзя перепродать сам
фреймворк как отдельный продукт.
