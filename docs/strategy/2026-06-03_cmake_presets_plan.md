...
| `crossbuild-essential-e2k` (apt-пакет) | **Не существует.** На Эльбрусе компилятор = LCC (`lcc`/`l++`), ставится `lcc-c++`/`gcc-elbrus` (на Альт) или идёт в составе MCST PDK. Никакого debian `crossbuild-essential-e2k` нет. Cross-build x86→e2k у нас вообще не практикуется (собираем нативно на ELBRUS_HOST/e16c). |
...
**Версия CMake:** presets v3 требует CMake ≥3.21. На ELBRUS_HOST — CMake 3.28 (из MEMORY),
...
| Тест-прогон: alt/astra локально, Эльбрус на ELBRUS_HOST, E16C на e16c.ru, Windows CUDA | 2–4 ч (зависит от доступности железа) |
...
Главная проверка — что `cmake --preset elbrus` на ELBRUS_HOST даёт те же `-march`/BLAS,
