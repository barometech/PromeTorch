...
pre-SINQ — **рабочий drop-in на Эльбрусе** (проверено). Чтобы извлечь
полную пользу: (а) Q5_K k-slice для паритета скорости, (б) harder eval
для подтверждения качества. Обе — follow-up. Сам факт что Huawei
Apache-2.0 квантизация загружается и генерит на 8C2 без единой строки
изменений — подтверждён.
