...
| llama.cpp upstream на этой же модели и железе | 1.82 tok/s (32t single-proc, tg64; см. `llama_cpp_bench_log.txt`) |
| **Ускорение PromeTorch / llama.cpp** | **×6.3** (11.4 / 1.82) |
...
Эльбрус 8C2 имеет 4 канала DDR4 на чип = **76.8 ГБ/с** при DDR4-2400
(68.3 при DDR4-2133; частота стенда не зафиксирована, см.
...
Это на **×6.3** быстрее llama.cpp (1.82 tok/s = 5.7 % от пика) — главное достижение
работы. Время одной decode-итерации:
- llama.cpp: ~550 мс / токен (один процесс на 32 ядрах не утилизирует ПСП)
...
на **×6.3** быстрее официального llama.cpp upstream на этой же машине. Это
