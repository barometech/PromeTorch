...
; do
    ...
    curl -s -m 600 -X POST http://127.0.0.1:11500/api/chat \
        ...
        --data-raw "{\"model\":\"/home/<user>/gguf_models/mistral-7b-Q4_K_M.gguf\",\"stream\":false,\"tools\":[],\"messages\":[{\"role\":\"user\",\"content\":\"$task\"}],\"options\":{\"num_predict\":700,\"temperature\":0.5,\"repeat_penalty\":1.1}}" \
        > "$out" 2>&1
