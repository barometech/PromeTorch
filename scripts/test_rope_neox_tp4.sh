...
run_tp4() {
    ...
    NPROCS=${PT_NPROCS:-4}; for ((r=0; r<NPROCS; r++)); do
      ...
      timeout 240 ./build_elbrus/examples/gguf/test_gguf_inference \
        "$model" \
        --nprocs ${PT_NPROCS:-4} --rank $r --master-addr 127.0.0.1 --master-port 29610 \
...
run_tp4 "mistral7b_ru" \
    /home/<user>/gguf_models/mistral-7b-Q4_K_M.gguf \
...
run_tp4 "qwen3_17b_ru" \
    /home/<user>/gguf_models/qwen3-1.7b-Q4_K_M.gguf \
    "Привет! Расскажи коротко про Москву на русском." \
    "PT_NO_THINK=1"
