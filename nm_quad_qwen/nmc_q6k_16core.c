/* Q6_K 4-core: runtime rank via ncl_getCoreID, single ABS shared by all cores */
#include <stdio.h>
#include <math.h>
#include <string.h>
...
#define M_TOTAL 32
#define M_PER_CORE 2  /* 16 cores total */
#define K 2560
...
static float fp16_to_fp32(unsigned int h) {
    unsigned int sign = (h & 0x8000) << 16; int exp = (h >> 10) & 0x1F; unsigned int mant = h & 0x3FF;
    unsigned int bits;
    if (exp == 0) { if (mant == 0) bits = sign; else { while ((mant & 0x400) == 0) { mant <<= 1; exp--; } mant &= 0x3FF; exp++; bits = sign | ((unsigned int)(exp + 127 - 15) << 23) | (mant << 13); } }
...
int main(int argc, char **argv) {
    ...
    int rank = ncl_getClusterID() * 4 + ncl_getCoreID();
