/* ============================================================================
 * host_q4k_gemv.cpp — Q4_K GEMV microbench on NMC4.
 * Tests M=32, K=256 (1 block per row, 32 rows × 144 bytes = 4608 bytes weights).
 * Uses first 32 rows of blk.0.attn_k.weight from Qwen3-4B Q4_K_M.
 *
 * Verifies NMC result vs host computation. Also times the kernel.
 * ============================================================================ */
...
#define M 32
#define K 256
#define NBYTES_W (M * 144)

/* Host-side Q4_K dequant + row-GEMV for verification */
...
static float q4k_gemv_row_host(const uint8_t *blk, const float *x) {
...
/* Find first Q4_K tensor offset in GGUF (same logic as host_q4k_test) */
struct GGUFReader {
    std::ifstream f;
    ...
    bool parse() {
        ...
        for (auto &t : tt) if (t.type == 12) {
            first_q4k_offset = base + t.off;
            std::cerr << "[gguf] " << t.name << " offset=" << first_q4k_offset << "\n";
            return true;
        }
        return false;
    }
...
int main(int argc, char *argv[]) {
    ...
    /* Load first 32 Q4_K blocks (32 rows × 144 = 4608 bytes) from Qwen */
    GGUFReader g;
    ...
    std::cerr << "[host] loaded " << NBYTES_W << " bytes of Q4_K weights\n";

    /* Random input x (deterministic seed) */
    ...
    for (int r = 0; r < M; ++r) y_host[r] = q4k_gemv_row_host(W.data() + r * 144, x.data());
    ...
    std::cout << "[verify] max_diff=" << maxd << " rms_diff=" << std::sqrt(sumsq / M) << "\n";
    std::cout << "[perf] kernel wall: " << ms << " ms\n";
    /* MAC count: M rows × K mac = 32×256 = 8192 MACs */
