/* ============================================================================
 * host_q4k_gemv_pthread.cpp — Q4_K GEMV K=2560 with proper pthread launch.
 * Each NMC4 core runs in its own pthread, all started simultaneously via
 * cond_broadcast. Pattern from /usr/local/rc_module/example/use_sync/host_pthread.c.
 *
 * 4 cores in cluster 0 each process 8 of 32 rows. Shared cluster EMI for W/x/y.
 * ============================================================================ */
...
static PL_Board *g_board = nullptr;
...
struct ThreadArg {
    int core_idx;     /* 0..3 */
    PL_Access *acc;   /* set after PL_GetAccess inside thread */
};

static void *core_worker(void *arg) {
    ThreadArg *t = (ThreadArg*)arg;

    PL_CoreNo core; core.cluster_id = 0; core.nm_id = t->core_idx;
    if (PL_GetAccess(g_board, &core, &t->acc) != PL_OK) {
        std::cerr << "[t" << t->core_idx << "] PL_GetAccess failed\n"; return nullptr;
    }
    if (PL_LoadProgramFile(t->acc, NMC_PART) != PL_OK) {
        std::cerr << "[t" << t->core_idx << "] PL_LoadProgramFile failed\n"; return nullptr;
    }

    pthread_mutex_lock(&g_mtx);
    g_loaded_count++;
    pthread_cond_broadcast(&g_cond);
    while (!g_release) pthread_cond_wait(&g_cond, &g_mtx);
    pthread_mutex_unlock(&g_mtx);

    /* All 4 threads launch simultaneously now */
    IO_Service *svc = IO_ServiceStart(NMC_PART, t->acc, nullptr, nullptr, nullptr);
    PL_Word st = 0;
    while (1) {
        if (PL_GetStatus(t->acc, &st) != PL_OK) break;
...
static float q4k_row_dot(const uint8_t *row, const float *x) {
    float a = 0;
    for (int b = 0; b < BLOCKS_PER_ROW; ++b) a += q4k_block_dot(row + b * 144, x + b * 256);
    return a;
}
...
int main(int argc, char *argv[]) {
    ...
    for (int r = 0; r < M_TOTAL; ++r) y_host[r] = q4k_row_dot(W.data() + r * ROW_BYTES, x.data());

    unsigned int n_b = 0;
    if (PL_GetBoardCount(&n_b) != PL_OK || n_b < 1) return 3;
    if (PL_GetBoardDesc(0, &g_board) != PL_OK) return 4;
    PL_ResetBoard(g_board); PL_LoadInitCode(g_board);

    /* Spawn 4 threads, each does PL_GetAccess + PL_LoadProgramFile then waits */
    pthread_t threads[NCORES];
    ThreadArg targs[NCORES];
    for (int c = 0; c < NCORES; ++c) {
        targs[c].core_idx = c;
        targs[c].acc = nullptr;
        pthread_create(&threads[c], nullptr, core_worker, &targs[c]);
    }

    /* Wait for all 4 to finish PL_LoadProgramFile */
    pthread_mutex_lock(&g_mtx);
    while (g_loaded_count < NCORES) pthread_cond_wait(&g_cond, &g_mtx);
    pthread_mutex_unlock(&g_mtx);

    /* Now safe to upload W/x to cluster 0 EMI (use any of the acc's, they share EMI) */
    std::vector<PL_Word> xw(K);
    std::memcpy(xw.data(), x.data(), K * 4);
    PL_WriteMemBlock(targs[0].acc, xw.data(), ADDR_X, K);

    int nbytes_w_full = M_TOTAL * ROW_BYTES;
    std::vector<PL_Word> ww(nbytes_w_full);
    for (int i = 0; i < nbytes_w_full; ++i) ww[i] = (PL_Word)W[i];
    PL_WriteMemBlock(targs[0].acc, ww.data(), ADDR_W, nbytes_w_full);

    /* Release all 4 threads to launch kernels simultaneously */
    ...
    for (int c = 0; c < NCORES; ++c) pthread_join(threads[c], nullptr);
    auto t1 = std::chrono::steady_clock::now();
    double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();

    /* Read y from shared EMI */
    std::vector<float> y_nmc(M_TOTAL);
    std::vector<PL_Word> yw(M_TOTAL);
    PL_ReadMemBlock(targs[0].acc, yw.data(), ADDR_Y, M_TOTAL);
    for (int i = 0; i < M_TOTAL; ++i) std::memcpy(&y_nmc[i], &yw[i], 4);
    ...
    std::cout << "[pthread-4c] wall: " << ms << " ms\n";
    std::cout << "[pthread-4c] max_diff=" << maxd << " (row " << worst_row
              << ") rms_diff=" << std::sqrt(sumsq / M_TOTAL) << "\n";
    double macs = (double)M_TOTAL * K;
    std::cout << "[pthread-4c] " << macs / (ms * 1e3) << " MMACs/sec across 4 cores\n";

    /* Cleanup */
    for (int c = 0; c < NCORES; ++c) if (targs[c].acc) PL_CloseAccess(targs[c].acc);
    PL_CloseBoardDesc(g_board);
