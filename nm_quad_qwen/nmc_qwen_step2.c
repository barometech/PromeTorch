/* ============================================================================
 * nmc_qwen_step2.c — Qwen forward STEP 2: RMSNorm + FULL Q-projection.
 *
 * Расширение step1: M=2560 (full Q-proj output dim) вместо 32.
 *
 *   y[K=2560] = RMSNorm(x, gamma)
 *   q[M=2560] = Q4_K_GEMV(y, W[M=2560, K=2560])
 *
 * Q-proj weights size: 2560 × 1440 = 3.7MB. Fits cluster EMI.
 * Timing: extrapolated from M=32 (24.9 ms) → M=2560 ≈ 2 sec на 1 ядро.
 *
 * Этот kernel = первая FULL attention sub-layer step Qwen 4B.
 * ============================================================================ */
...
float        x[K];
float        gamma_v[K];
unsigned int W[M * ROW_WORDS];     /* 3.7MB Q4_K weights */
float        y[K];
float        q[M];
...
int main(int argc, char *argv[]) {
    ...
    printf("NMC%d:%d step2 done: M=%d K=%d inv_rms=%f q[0..3]=%f %f %f %f\n",
        cluster, core, M, K, inv_rms, q[0], q[1], q[2], q[3]);
    printf("  q[2556..2559]=%f %f %f %f\n", q[2556], q[2557], q[2558], q[2559]);
    return 0;
}
