...
int main(int argc, char *argv[]) {
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        char name[64]; std::snprintf(name, sizeof(name), "./nmc%d_part.abs", c);
        if (PL_LoadProgramFile(acc[c], name) != PL_OK) return 5;
    }
    ...
    for (int c = 0; c < 4; ++c) {
        char name[64]; std::snprintf(name, sizeof(name), "./nmc%d_part.abs", c);
        svc[c] = IO_ServiceStart(name, acc[c], nullptr, nullptr, nullptr);
    }
    for (int c = 0; c < 4; ++c) {
        PL_Word st = 0;
